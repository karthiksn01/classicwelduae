...
class ProductUpdaterController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (empty($data['warranty_years'])) $data['warranty_years'] = 0;
        ...
        if ($request->hasFile('image_file')) {
            ...
            \Illuminate\Support\Facades\Log::info('Attempting upload to: ' . $destinationPath);
            
            try {
                $file->move($destinationPath, $fileName);
                $data['image'] = '/api/uploads/products/' . $fileName;
                \Illuminate\Support\Facades\Log::info('Upload successful: ' . $data['image']);
            } catch (\Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if (isset($data['stock']) && $data['stock'] === null) $data['stock'] = 0;
        if (isset($data['warranty_years']) && $data['warranty_years'] === null) $data['warranty_years'] = 0;
        ...
        if ($request->hasFile('image_file')) {
            // Move file to uploads folder using robust path detection
            ...
            $product->image = '/api/uploads/products/' . $fileName;
        }
