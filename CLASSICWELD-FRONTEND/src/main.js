  ...
  try {
    ...
    products.forEach((product) => {
      ...
      const cartButton = isSoldOut 
        ...
        : `<button onclick="addToCart(${product.id}, '${product.name.replace(/'/g, "\\'")}', 0, '${imgSrc}')" class="w-full bg-weld-orange hover:bg-amber-600 text-white text-sm font-bold py-2 rounded-lg transition-colors flex items-center justify-center gap-1">
      ...
      const card = `
          <div class="glass rounded-xl overflow-hidden group border border-white/5 hover:border-weld-orange/50 transition-all duration-300 flex flex-col h-full relative ${isSoldOut ? 'opacity-75 grayscale-[0.5]' : ''}">
              ...
              <div class="relative h-48 overflow-hidden bg-zinc-800 cursor-pointer" onclick="window.location.href='product-detail.html?id=${product.id}'">
                  ${soldOutOverlay}
              ...
              <div class="p-5 flex-1 flex flex-col">
                  ...
                  <h3 class="text-lg font-bold mb-2 ${isSoldOut ? '' : 'group-hover:text-weld-orange'} transition-colors cursor-pointer line-clamp-2" onclick="window.location.href='product-detail.html?id=${product.id}'">${product.name}</h3>
                  ...
                  <div class="grid grid-cols-2 gap-2 mt-auto relative z-30">
                      <button onclick="window.location.href='product-detail.html?id=${product.id}'" class="w-full bg-zinc-800 hover:bg-zinc-700 text-white text-sm font-bold py-2 rounded-lg transition-colors border border-zinc-700">Details</button>
                      ${cartButton}
                  </div>
              </div>
          </div>
            ...
            html += `
                <div class="w-full h-full flex-shrink-0 flex flex-col md:flex-row bg-zinc-900">
                    <div class="w-full md:w-1/2 p-6 md:p-12 flex flex-col justify-center relative z-20">
                       ...
                       <div class="flex gap-4">
                           <button onclick="window.location.href='product-detail.html?id=${product.id}'" class="bg-weld-orange hover:bg-amber-600 text-white px-8 py-4 rounded-lg font-bold transition-all shadow-lg shadow-amber-500/30 flex items-center gap-2">
                               View Details <i class="ph ph-arrow-right"></i>
                           </button>
                       </div>
                    </div>
                    <div class="w-full md:w-1/2 h-full cursor-pointer relative overflow-hidden group/carousel" onclick="window.location.href='product-detail.html?id=${product.id}'">
            ...
            try {
                ...
                if (products.length > 0) {
                    ...
                    products.forEach(p => {
                        ...
                        html += `
                            <a href="product-detail.html?id=${p.id}" class="flex items-center gap-3 p-3 hover:bg-zinc-800 transition-colors border-b border-white/5 last:border-0 no-underline">
                    ...
                    html += `
                        <a href="products.html?search=${encodeURIComponent(query)}" class="block p-3 text-center text-xs font-bold text-weld-orange hover:bg-zinc-800 transition-colors bg-black/20 no-underline">
                            See all results
                        </a>
                    `;
